...
class ProGenAttention(nn.Module):
    def __init__(self, config):
        ...
        self._pre_quantization_dtype = getattr(config, "_pre_quantization_dtype", None)
    ...
    def _naive_attn(
        ...
        if attn_output.size() != expected_size:
            raise ValueError(f"`attn_output` should be of size {expected_size}, but is {attn_output.size()}")
    ...
    def forward(
        ...
        elif self._pre_quantization_dtype is not None:
            target_dtype = self._pre_quantization_dtype
        else:
...
class ProGenModel(ProGenPreTrainedModel):
    ...
    def forward(
        ...
        # Attention mask. Not used for SDPA, since we can hardcode a causal mask there.
        if attention_mask is not None and not output_attentions:
...
class ProGenForCausalLM(ProGenPreTrainedModel, GenerationMixin):
    ...
    def prepare_inputs_for_generation(self, input_ids, past=None, **kwargs):
        ...
        past = kwargs.get("past_key_values", past)  # change to past_key_values for transformers>=4.26
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # Flatten the tokens
            loss_fct = CrossEntropyLoss(ignore_index=0)  # hardcoded pad token ID
        ...
        return CausalLMOutputWithPast(
            ...
            inputs=transformer_outputs.inputs,
        )
